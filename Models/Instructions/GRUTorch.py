...
class GRUTorch(nn.Module):
    ...
    def __init__(self, state_size: int, embedding_dimension: int, vocabulary_size: int, layers: int = 1):
        ...
        self.gru = nn.GRU(embedding_dimension, state_size, layers, batch_first=True)
        self.embedding = nn.Embedding(vocabulary_size, embedding_dimension)
        self.output = nn.Linear(state_size, vocabulary_size)

    def forward(self, x: torch.Tensor, hidden_states = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        """
        x = self.embedding(x)
        x, hidden_states = self.gru(x, hidden_states if hidden_states is not None else None)
        x = self.output(x)
...
class GRUTrainer:
    def __init__(self, hidden_size: int, embedding_dimension: int, context_length: int, layers: int = 1):
        """Initializes a GRU model with the specified arguments.
        ...
        self.model = GRUTorch(hidden_size, embedding_dimension, enc.n_vocab + 1, layers)
    ...
    def train(self, max_epochs: int = 20, max_time: int = -1, max_iterations: int = -1, batch_size: int = 8) -> List[List[float]]:
        ...
        self._prepare()
        self.model.train()
        test = list(self.model.parameters())
    ...
    def save_model(self):
        torch.save(self.model.state_dict(), "./Models/Instructions/GRUTorch.pkl")

    def load_model(self):
        state_dict = torch.load("./Models/Instructions/GRUTorch.pkl")
